...
class TestDatabaseErrorBoundary:
    """PostgreSQL persistence/read failures map to domain errors."""
    ...
    def test_repository_read_error_raises_internal_error(self, test_db, monkeypatch):
        def _boom(*a, **k):
            raise RuntimeError("connection failed")
        monkeypatch.setattr(
            "app.services.result_repository.get_assessment_sync", _boom
        )
        with pytest.raises(AssessmentInternalError):
            get_assessment_result("any-task-id")

    def test_no_sensitive_info_in_read_error(self, test_db, monkeypatch):
        secret = "secret_db_error_at /tmp/private/database.db"
        ...
        with pytest.raises(AssessmentInternalError) as exc:
            get_assessment_result("any-task-id")
...
class TestSaveAssessmentDatabaseBoundary:
    """PostgreSQL persistence failures map to AssessmentPersistError."""

    def test_persist_error_on_repository_failure(self, test_db, monkeypatch):
        task_id, scan_dict, scan_updated = _setup_task_with_scan(test_db)
        assessment = assess_scan_result(task_id, scan_dict)
        ...
        monkeypatch.setattr(
            "app.services.result_repository.save_assessment_sync", _boom
        )
        with pytest.raises(AssessmentPersistError) as exc:
            save_assessment_result(task_id, assessment, scan_updated)
        assert "connection failed" not in str(exc.value)

    def test_serialization_errors_are_not_persist_errors(self, test_db, monkeypatch):
        task_id, scan_dict, scan_updated = _setup_task_with_scan(test_db)
        with pytest.raises(AssessmentSerializationError):
            save_assessment_result(task_id, {"task_id": "wrong"}, scan_updated)



class TestGetAssessmentCloseSemantics:
    """PostgreSQL read path: missing row → None; DB failure → InternalError."""

    def test_row_not_found_returns_none(self, test_db):
        result = get_assessment_result("nonexistent-task-id")
        assert result is None

    def test_repository_failure_raises_internal_error(self, test_db, monkeypatch):
        task_id, scan_dict, scan_updated = _setup_task_with_scan(test_db)
        assessment = assess_scan_result(task_id, scan_dict)
        save_assessment_result(task_id, assessment, scan_updated)
