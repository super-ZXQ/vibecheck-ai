...
- **每用户 LLM 配置**：用户可在页头填写自己的 LLM API Key / Base URL / 模型，凭据仅存于浏览器 localStorage，随检测请求以 `X-LLM-*` 请求头发送，后端仅在进程内存中用于本次检测的 LLM 分析阶段，不落库、不记录日志、不返回给任何调用方，任务结束即释放。
...
- **超限引导**：GitHub 检测因仓库超限失败（`DOWNLOAD_TOO_LARGE` / `EXTRACTION_LIMIT_EXCEEDED`）时，失败页显示「改用本地上传」引导，一键回到首页并高亮上传卡片。
...
`POST /api/check` 与 `POST /api/check/upload` 均可携带以下可选请求头，将用户自己的 LLM 凭据绑定到本次任务（前端在「LLM 设置」弹窗中填写并存储于浏览器本地）：

| 请求头 | 说明 |
| --- | --- |
| `X-LLM-API-KEY` | 用户自己的 LLM API Key（≤ 1000 字符） |
| `X-LLM-BASE-URL` | OpenAI 兼容端点地址（必须为 http(s)，禁止内嵌凭据，≤ 500 字符） |
| `X-LLM-MODEL` | 模型名称（≤ 200 字符） |

安全约束：凭据仅存于服务端进程内存（TTL 4 小时兜底），不落库、不写日志、不返回；任务完成或失败后立即释放。API Key / Base URL / 模型三者都填才会覆盖服务端配置（此时即使服务端未启用 LLM 也会使用用户凭据生成 AI 分析）；只填一部分则忽略并回退服务端配置或模板。
